...
async function startHisoka() {
    ...
    hisoka.sendMessage(callerId, { text: `「 *Automatisches Blocksystem* 」\n\nVerzeihung!\n*HISOKA* -ChatBot ist kein CallBot.\nUm Belastung zu vermeiden, folgt nun ein BLOCK\nEin Block ist kein BAN, du kannst weiterhin Befehle in Gruppen tätigen!\n\n\nBitte Kontaktiere einen der aufgelisteten administratoren für weitere informationen. Oder schreiben sie einen "Entbannungsantrag" an support@discord-home.dev`}, { quoted : pa7rick })
    await sleep(8000)
    ...
    // Get Profile Picture Group
       ...
       hisoka.send5ButImg(pea[0].id, `「 Gruppen Einstellung Geändert 」\n\nDie Gruppeninfo wurde geöffnet, jetzt können die Teilnehmer die Gruppeninfo bearbeiten!`, `Nachricht zur Änderung der Gruppeneinstellungen`, wm_fatih, [])
       } else {
    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `「 *Willkommen* 」\n\nWillkommen in _${metadata.subject}_ viel spaß @${num.split("@")[0]}.` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `「 *Tschüss* 」\n\n@${num.split("@")[0]} hat gerade _${metadata.subject}_ verlassen... bye!` })
                }
